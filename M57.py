...
hdulist = fits.open('M57.fits') #fetch the file

#getting necessary header values
photflam = hdulist[1].header['PHOTFLAM'] # inverse sensitivity of telescope
exptime = hdulist[0].header['EXPTIME']  # total exposure time


#Get data in correct units
img = CCDData.read('M57.fits', unit='adu')  # get data from the file
...
cut_ctr = SkyCoord('18h53m35.079s 33d1m45.03s') 
cut_dims = np.array([1.75, 1.75]) * u.arcmin
...
trans = viz.LogStretch() + viz.ManualInterval(0, 3e-16)
...
plt.suptitle('M57 --- Ring Nebula', weight = 'bold', size = 16)
plt.show()